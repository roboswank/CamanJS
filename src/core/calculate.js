...
class Calculate {
	...
	static bezier(start, ctrl1, ctrl2, end, lowBound = 0, highBound = 255) {
		...
		for (i = k = 0; k < 1000; i = ++k) {
			...
			while (prev.length > 1) {
				next = [];
				for (j = l = 0, ref = prev.length - 2; undefined !== 0 && (0 <= ref ? 0 <= l && l <= ref : 0 >= l && l >= ref); j = 0 <= ref ? ++l : --l) {
	...
	static hermite(controlPoints, lowBound, highBound) {
		...
		count = 0;
		for (i = k = 0, ref = controlPoints.length - 2; undefined !== 0 && (0 <= ref ? 0 <= k && k <= ref : 0 >= k && k >= ref); i = 0 <= ref ? ++k : --k) {
			p0 = controlPoints[i];
			...
			for (j = l = 0, ref1 = pointsPerSegment; undefined !== 0 && (0 <= ref1 ? 0 <= l && l <= ref1 : 0 >= l && l >= ref1); j = 0 <= ref1 ? ++l : --l) {
				t = j * pointsPerStep;
	...
	static missingValues(values, endX) {
		...
		if (Object.keys(values).length < endX + 1) {
			ret = {};
			for (i = k = 0, ref = endX; undefined !== 0 && (0 <= ref ? 0 <= k && k <= ref : 0 >= k && k >= ref); i = 0 <= ref ? ++k : --k) {
				if (values[i] != null) {
					ret[i] = values[i];
				} else {
					...
					// very quickly.
					for (j = l = ref1 = i, ref2 = endX; undefined !== 0 && (ref1 <= ref2 ? ref1 <= l && l <= ref2 : ref1 >= l && l >= ref2); j = ref1 <= ref2 ? ++l : --l) {
						if (values[j] != null) {
